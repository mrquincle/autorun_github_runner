...
#
# Packages needed for ssh access (and some other tools)
#
echo "--------------------------------------------------" >> "${MY_LOGFILE}"
...
#
echo "--------------------------------------------------" >> "${MY_LOGFILE}"
echo '** Configuring SSH access...' | tee -a "${MY_LOGFILE}"

ALREADY_IN_TARGET=""
HOST_IP=192.168.50.218

log_command_in_target /bin/bash -c "mkdir -p ${ALREADY_IN_TARGET}/etc/ssh/sshd_config.d"
log_command_in_target /bin/bash -c "touch ${ALREADY_IN_TARGET}/etc/ssh/sshd_config.d/custom.conf"
log_command_in_target /bin/bash -c "echo 'AllowUsers @@VBOX_INSERT_USER_LOGIN@@' >> ${ALREADY_IN_TARGET}/etc/ssh/sshd_config.d/custom.conf"
log_command_in_target /bin/bash -c "wget http://${HOST_IP}/userkey.pub"
log_command_in_target /bin/bash -c "mkdir -p ${ALREADY_IN_TARGET}/home/@@VBOX_INSERT_USER_LOGIN@@/.ssh"
log_command_in_target /bin/bash -c "touch ${ALREADY_IN_TARGET}/home/@@VBOX_INSERT_USER_LOGIN@@/.ssh/authorized_keys"
log_command_in_target /bin/bash -c "cat userkey.pub >> ${ALREADY_IN_TARGET}/home/@@VBOX_INSERT_USER_LOGIN@@/.ssh/authorized_keys"
log_command_in_target /bin/bash -c "chmod 600 ${ALREADY_IN_TARGET}/home/@@VBOX_INSERT_USER_LOGIN@@/.ssh/authorized_keys"
log_command_in_target /bin/bash -c "chown @@VBOX_INSERT_USER_LOGIN@@: ${ALREADY_IN_TARGET}/home/@@VBOX_INSERT_USER_LOGIN@@/.ssh"
log_command_in_target /bin/bash -c "chown @@VBOX_INSERT_USER_LOGIN@@: ${ALREADY_IN_TARGET}/home/@@VBOX_INSERT_USER_LOGIN@@/.ssh/authorized_keys"

# Also add to sudoers
log_command_in_target /bin/bash -c "echo '@@VBOX_INSERT_USER_LOGIN@@ ALL=(ALL) ALL' >> ${ALREADY_IN_TARGET}/etc/sudoers"

# In chroot, restart does not work
#log_command_in_target /bin/bash -c "systemctl restart sshd"
